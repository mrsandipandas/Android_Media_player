 ...
 * retrieval, such as if authorization is required to download the key.  In this
...
extern "C"
{
     ...
     *
     * This can be registered by calling 
     * ::nexPlayerSWP_RegisterGetPlaylistInfoCallBackFunc
     *
     ...
     *
     * \param[out]  pBuf            The buffer to receive the new credentials string
     *
     * \param[in]   dwBufSize       The size, in bytes, of the buffer at \c pBuf
     *
     * \param[out]  pdwNewBufSize   The number of bytes needed to store the headers,
     ...
     * 
     *  This callback can be registered by calling
     *  ::nexPlayerSWP_RegisterGetPDBlockCallBackFunc.
     *
...
#define NXWMDRM_FRAME_TYPE      0x20
     ...
     *     - a non-zero error code if the operation failed
     *
     * \see ::NEXPLAYERGetPlaylistInfoCallbackFunc
     */
    ...
    /**
     *  \ingroup apifunc
     *  \brief Registers a callback function to receive pointer for Progressive Download(PD) block and block's size.
     ...
     *     - a non-zero error code if the operation failed 
     *
     * \see ::NEXPLAYERGetPDBlockCallbackFunc
    **/
